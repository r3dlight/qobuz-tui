...
use crate::app::{App, LoginField, Screen, SearchMode, Tab, scroll_offset};
use ratatui::Frame;
...
fn render_login(f: &mut Frame, app: &App) {
    ...
    for (i, (label, field)) in labels.iter().zip(fields.iter()).enumerate() {
        ...
        f.render_widget(
            Paragraph::new(format!("  {}", label)).style(lbl_style),
            chunks[label_idx],
        );
        ...
        f.render_widget(
            Paragraph::new(format!(" {}{}", display, cursor))
                .style(Style::default().fg(WHITE).bg(input_bg)),
            chunks[input_idx],
        );
    }

    // Submit
    let (sub_fg, sub_bg) = if app.login_focus == LoginField::Submit {
        (SURFACE, BRAND)
    } else {
        (BRAND, SURFACE_2)
    };
    let sub_text = if app.login_loading {
        format!(" {} Connecting... ", ICON_LOADING)
    } else {
        "  \u{2192} Sign In  ".to_string()
    };
    f.render_widget(
        ...
        Tab::Playlists => {
            ...
            f.render_widget(
                Paragraph::new(Line::from(vec![
                    ...
                    Span::styled(
                        format!("{} playlist{}", n, if n != 1 { "s" } else { "" }),
                        Style::default().fg(TEXT),
                    ),
                ]))
                .block(
...
fn render_search(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if let Some(status) = &app.status_message {
        ...
        spans.extend([
            Span::styled("   ", Style::default()),
            Span::styled(status, Style::default().fg(c)),
        ]);
    }
...
fn render_track_list(
    ...
    let rows: Vec<Row> = tracks[offset..end]
        ...
        .map(|(vi, track)| {
            ...
            if show_album {
                cells.push(
                    Cell::from(track.album_title())
                        .style(Style::default().fg(if sel { GOLD } else { DIM }).bg(bg)),
                );
            }
            cells.push(
                Cell::from(track.format_duration())
                    .style(Style::default().fg(if sel { GOLD } else { DIM }).bg(bg)),
            );
    ...
    let scroll = if tracks.len() > visible {
        format!(" {}-{}/{} ", offset + 1, end, tracks.len())
    } else {
        String::new()
    };
    f.render_widget(
        Table::new(rows, widths)
            .header(header)
            .block(panel_scroll(title, scroll)),
        area,
    );
}
...
fn render_album_list(
    ...
    let scroll = if albums.len() > visible {
        format!(" {}-{}/{} ", offset + 1, end, albums.len())
    } else {
        String::new()
    };
    f.render_widget(
...
fn render_favorites(f: &mut Frame, app: &mut App, area: Rect) {
    if !app.favorites_loaded {
        f.render_widget(
            Paragraph::new(format!(" {} Loading...", ICON_LOADING))
                .style(Style::default().fg(GOLD))
                .alignment(Alignment::Center)
                .block(panel(" Favorites ")),
            area,
        );
        return;
    }
    ...
    render_album_list(
        f,
        &app.favorite_albums,
        app.favorites_selected,
        app.favorites_scroll,
        area,
    );
}
...
fn render_playlists(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    .style(Style::default().fg(BRAND).add_modifier(Modifier::BOLD));
    let rows: Vec<Row> = app.playlists[offset..end]
        .iter()
        .enumerate()
        .map(|(vi, pl)| {
            let i = offset + vi;
        ...
        f.render_widget(
            ...
            .block(
                Block::default()
                    ...
                    .title_bottom(if bio.is_empty() {
                        Line::from("")
                    } else {
                        Line::from(Span::styled(
                            format!(" {} ", &bio[..bio.len().min(80)]),
                            Style::default().fg(MUTED),
                        ))
                    }),
            ),
        ...
        f.render_widget(
            Paragraph::new(vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("  \u{2022}  {} tracks  \u{2022}  {}", tc, year),
                        Style::default().fg(DIM),
                    ),
                ]),
            ])
...
fn render_player_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    if app.player.current_track_title.is_some() {
        let seekable = if app.player.is_seekable() {
            "seekable"
        } else {
            "streaming"
        };
...
fn render_status(f: &mut Frame, app: &App, area: Rect) {
    if let Some(s) = &app.status_message {
        let c = if app.status_is_error { RED } else { GOLD };
        f.render_widget(
            Paragraph::new(format!(" {}", s)).style(Style::default().fg(c)),
            area,
        );
    }
}
